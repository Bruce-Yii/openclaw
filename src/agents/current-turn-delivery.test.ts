...
function createCurrentTurnDeliveryHarness(params?: {
  ...
  beforeAdapter?: (token: string) => void;
    ...
    createTestRegistry([
      {
        ...
        plugin: createOutboundTestPlugin({
          ...
          outbound: {
            deliveryMode: "direct",
            ...(params?.beforeAdapter
              ? {
                  normalizePayload: ({ payload }) => {
                    params.beforeAdapter?.(token);
                    return payload;
                  },
                }
              : {}),
...
describe("current-turn delivery tool", () => {
  ...
  it.each(["revoked", "replaced"] as const)(
    "rejects %s authority at the final fence before adapter I/O",
    async (mode) => {
      const { current, sendText, token } = createCurrentTurnDeliveryHarness({
        beforeAdapter: (activeToken) => {
          if (mode === "revoked") {
